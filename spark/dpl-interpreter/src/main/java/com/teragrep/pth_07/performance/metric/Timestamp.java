...
public final class Timestamp implements PerformanceMetric<Long> {
    ...
    @Override
    public Timestamp withValue(java.lang.Object value) throws IncompatibleValueException {
        final Timestamp modifiedMetric;
        if(value instanceof String){
            try{
                long newValue = Long.parseLong((String)value);
                modifiedMetric = new Timestamp(newValue);
            }
            ...
            modifiedMetric = new Timestamp((Long)value);
        }
        else {
            throw new IncompatibleValueException("Value "+value+" is not a compatible value for metric "+name());
        }
